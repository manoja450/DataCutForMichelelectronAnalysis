...
void processEvents(const char *fileName) {
    ...
    vector<vector<Double_t>> goodPulseRatios, badPulseRatios; // Store Pulse Area / Pulse Height ratios

    // Loop over all events for selection
    for (Long64_t entry=0; entry<nEntries; entry++) {
        tree->GetEntry(entry);

        // Check if the event does NOT satisfy the initial criteria (at least 3 PMTs with pulse height > 2 PE)
        int countAbove2PE = 0;
        for (int pmt=0; pmt<12; pmt++) {
        ...
        if (countAbove2PE < 3) {
            // Now check the new criteria for these events
            int countNewCriteria = 0;
            vector<Double_t> pulseRatios(12, 0.0); // Store Pulse Area / Pulse Height ratios for this event

            for (int pmt=0; pmt<12; pmt++) {
                // Calculate Pulse Area / Pulse Height ratio
                if (pulseH[pmtChannelMap[pmt]] > 0) {
                    pulseRatios[pmt] = area[pmtChannelMap[pmt]] / pulseH[pmtChannelMap[pmt]];
                }

                // Check new criteria
                if (pulseH[pmtChannelMap[pmt]] > 3 * baselineRMS[pmtChannelMap[pmt]] &&
                    pulseRatios[pmt] > 1) {
                    countNewCriteria++;
                }
            }

            // Categorize events based on new criteria
            if (countNewCriteria >= 3) {
                goodEvents.push_back(entry); // Good event
                goodPulseRatios.push_back(pulseRatios); // Store ratios
            } else {
                badEvents.push_back(entry); // Bad event
                badPulseRatios.push_back(pulseRatios); // Store ratios
            }
        }
    }
    ...
    // Add a new branch for Pulse Area / Pulse Height ratios
    Double_t pulseRatios[12];
    goodTree->Branch("PulseRatios", pulseRatios, "PulseRatios[12]/D");

    for (size_t i=0; i<goodEvents.size(); i++) {
        tree->GetEntry(goodEvents[i]); // Load event data
        for (int pmt=0; pmt<12; pmt++) {
            pulseRatios[pmt] = goodPulseRatios[i][pmt]; // Add ratios
        }
    ...
    // Add a new branch for Pulse Area / Pulse Height ratios
    badTree->Branch("PulseRatios", pulseRatios, "PulseRatios[12]/D");

    for (size_t i=0; i<badEvents.size(); i++) {
        tree->GetEntry(badEvents[i]); // Load event data
        for (int pmt=0; pmt<12; pmt++) {
            pulseRatios[pmt] = badPulseRatios[i][pmt]; // Add ratios
        }
